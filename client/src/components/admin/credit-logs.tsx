...
export function CreditLogs() {
  ...
  const { data: creditEmails = [], isLoading: isLoadingCreditEmails } = useQuery<CreditEmail[]>({
    queryKey: ["/api/admin/credit-notifications"],
    enabled: !!token,
    refetchInterval: 5000,
  });
  ...
  const retryCreditEmail = useMutation({
    mutationFn: (transactionId: string) => apiRequest("POST", `/api/admin/credit-notifications/${transactionId}/retry`).then(response => response.json()),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/credit-notifications"] });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/credit-transactions"] });
      toast({ title: "Credit email queued", description: "The email will be retried with the current SMTP settings." });
    },
    onError: (error: Error) => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/credit-notifications"] });
      toast({ title: "Could not retry credit email", description: error.message, variant: "destructive" });
    },
  });
    ...
    <Card>
      <CardContent className="p-4 space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div><h3 className="font-semibold flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-amber-500" />Exhausted account-credit emails</h3><p className="text-sm text-muted-foreground mt-1">Credit notifications that failed five delivery attempts. Retrying only sends the existing email again.</p></div>
          <Badge variant="outline">{creditEmails.length}</Badge>
        </div>
        {isLoadingCreditEmails ? <div className="p-8 text-center text-muted-foreground">Loading exhausted emails…</div> : creditEmails.length === 0 ? <div className="rounded-md border border-dashed p-8 text-center text-muted-foreground">No exhausted account-credit emails.</div> : <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left text-muted-foreground"><th className="p-3">Failed at</th><th className="p-3">User</th><th className="p-3">Credit change</th><th className="p-3">Attempts</th><th className="p-3 min-w-[240px]">Last delivery error</th><th className="p-3 text-right">Action</th></tr></thead><tbody>{creditEmails.map(email => <tr key={email.id} className="border-b border-border/60 last:border-0 hover:bg-muted/30"><td className="p-3 whitespace-nowrap text-muted-foreground">{new Date(email.notificationAttemptedAt || email.createdAt).toLocaleString()}</td><td className="p-3"><div className="font-medium">{email.userEmail}</div><div className="text-xs text-muted-foreground">{email.type.replaceAll("_", " ")}</div></td><td className={`p-3 font-semibold whitespace-nowrap ${email.amountCents >= 0 ? "text-green-500" : "text-destructive"}`}>{email.amountCents >= 0 ? "+" : "−"}{money(email.amountCents)}</td><td className="p-3"><Badge variant="destructive">{email.notificationAttempts}/5</Badge></td><td className="p-3 max-w-[320px]"><span className="break-words text-destructive">{email.notificationLastError || "Unknown delivery error"}</span></td><td className="p-3 text-right"><Button size="sm" variant="outline" onClick={() => retryCreditEmail.mutate(email.id)} disabled={retryCreditEmail.isPending}><RefreshCw className={`mr-2 h-4 w-4 ${retryCreditEmail.isPending ? "animate-spin" : ""}`} />Retry</Button></td></tr>)}</tbody></table></div>}
      </CardContent>
    </Card>
